...
/*
2017apr06 - Dave Johnson - fiveangle@gmail.com
-Optimized for print speed and reduced material usage:
  -Removed all wire channels which allowed making the base 3 full layers (@0.350mm) thinner (as such, all wiring must be routed externally)
  -Removed the double slots for battery terminals, allowing the wire to simply fold over the top
  -Extended cylindrical bump designed to protrude wire for battery terminal to front-back to allow one to wrap the wire around the top and twist around itself more easily
  -Reduced overally height to exactly mid-level (giving up ability to stack - mine can be used only single-layer)
  -Redesigned spring to remove double-width unsprung area (reducing width of each slot by 4 ew)
  -Removed side and bottom grooves
  -Reduced walls between adjacent batteries to 2 shells (@0.45ew)
  -Removed text and battery markings
  -Reduced wall height using a modified version of enif's "grip" feature
  -Probably plenty of other things and small bugfixes I don't remember
  -This file is hacked together to get only the 18650 battery to work so is not longer generic (but feel free to fix if you want :)
*/
...
module flexbatter(n = 1, l = 65, d = 18, hf = 0.75, r = 4, shd = 3, eps = 0.28, el = 0, xchan = [1 / 4, 3 / 4], $fn = 24) {
    ...
    eh = 0.35; // extrusion height
    w = 2 * ew; // case wall thickness
    ws = 2 * ew; // spring wall thickness
    ch = ws - ws; // edge chamfering
    deepen = 1; //deepening for side grip of batteries
    ...
    for (i = [0: n - 1]) translate([0, i * (d + w + ws - w), 0]) { // generate n battery cases
        difference() {
            union() {
                difference() {
                    // main body
                    // translate([0, -w - d / 2, 0]) chamfered_cube([l + w, d + 2 * w + (i < n - 1 ? ws : 0), hf * d + w + ch], ch);
                    translate([0, -w - d / 2, 0]) chamfered_cube([l + w, d + 2 * w + (i < n - 1 ? 0 : 0), hf * d + w + ch], ch);
                    // main cavity
                    translate([-2, -d / 2, w]) cube([2 + l, d, d + 1]);
                    // cavity for locating plastic spring
                    translate([-1, -d / 2, -1]) cube([1, d, d + w + 2]);
            ...
            for (z = [-2 * ws, 2 * ws])
                translate([-2 * ws, -w, w - ws / 2 + d / 2 + z]) cube([l + 2 * w + 2, 2 * w, 2 * ws]);
            ...
            // grip deepening
            if (deepen > 0)
                translate([w + l / 2 - 0.5, -d / 2 - w - 0.01, w + d + l + (3.3)]) {
                    rotate([-90, 0, 0]) {
                        ...
                        if (i == n - 1) translate([0, 0, d + 2 * w - ch]) cylinder(r2 = l + deepen * d + ch, r1 = l + deepen * d, h = ch + 0.02, $fn = 72);
                                        ...
                                        // cap angle from deepen operation
                    translate([-(w + l / 2 - 0.5), -d / 2 - w - 0.01, d - (7.6)]) {
                        cube([100, d + 2 * w + 2 * ws + 2, 100]);
                    }
	 ...
	 // engrave minus symbol
        ...
        /*
                 if(i<n-1) translate([-d,d/2+w,-1])cube([d,ws/2+0.1,d+2]);
...
//flexbatterC(n=1);
//flexbatterD(n=1);
//translate([0,40,0])flexbatter18650(n=8);
flexbatter18650(n = 1);